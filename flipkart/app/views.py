...
def electronics_search(req):
    if req.method=="GET":
        ...
        # allproducts=Products.objects.filter(categories=ele_category)
...
def addtowishlist(req, productid):
    if req.user.is_authenticated:
        ...
        messages.error(req,'You need to log in to add products to your wishlist')
        return redirect('signin')
...
def addprofile(req):
    ...
    if req.method=="POST":
        ...
        if dob:
            ...
            print(age)
            if age<18:
                messages.error(req,"You must be atleast 18 years old to create profile")
